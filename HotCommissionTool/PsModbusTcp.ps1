...
function Send-ModbusCommand {
	...
	if ((New-Object System.Net.Sockets.TcpClient).ConnectAsync($Address, $Port).Wait(1500))
	{
...
function Read-HoldingRegisters {
    ...
    try {return Read-Registers ReadHoldingRegisters $Address $Port $Reference $Num $Slave}
	catch {Write-Host "$Address connect fail"}
}
